...
function accessLevelLabel(accessLevel) {
  if (accessLevel === "read_only") return "Read only";
  if (accessLevel === "full") return "Full";
  return "Scoped";
}

export async function getAccessRolesData() {
  const actor = await requirePermission("admin.roles", ["admin"]);
  ...
  return {
    readOnly: actor.access_role_level === "read_only",
    roles: rolesResult.rows.map((role) => ({
      ...
      accessLevel: accessLevelLabel(role.access_level),
