...
export async function syncAdminNotifications(client) {
  await client.query(
    ...
    SELECT
      ...
      'bookings',
      b.id,
      CONCAT('New booking from ', UPPER(LEFT(staff.role, 1)), LOWER(SUBSTRING(staff.role, 2)), ' staff'),
  ...
  await client.query(
    ...
    SELECT
      ...
      'payout_requests',
      pr.id,
      CONCAT('New payout request from ', UPPER(LEFT(requester.role, 1)), LOWER(SUBSTRING(requester.role, 2)), ' staff'),
