...
export interface TransactionOverviewPoint {
  date: string;
  expense: number;
  income?: number;
}
...
export function TransactionsOverviewCard({
  ...
  const firstDate = new Date(chartData[0]?.timestamp ?? Date.now());
  const weekStart = Date.UTC(firstDate.getUTCFullYear(), firstDate.getUTCMonth(), firstDate.getUTCDate());
