...
import { markAccessRolesChanged, requireSystemAccessRole } from "@/lib/access-role-write";
...
export async function POST(request) {
  try {
    ...
    const staff = await transaction(async (client) => {
      ...
      const accessRole = await requireSystemAccessRole(client, data.role);

      const { rows } = await client.query(
        `INSERT INTO users (name, email, phone, role, status, resort_id, access_role_id, password_hash)
         VALUES ($1, $2, $3, $4, $5, $6, $7, $8)
         RETURNING id, name, email, phone, role, status, resort_id, access_role_id, created_at`,
        [
          data.name,
          data.email,
          data.phone || null,
      ...
      await markAccessRolesChanged(client, [accessRole.id], actor.id);
