  ...
  try {
    ...
    return Response.json({
      users: users.rows.map((user) => ({
        ...user,
        presence: ['internal', 'external'].includes(user.role)
          ? user.status === 'active'
...
export async function POST(request) {
  try {
    ...
    if (!parsed.success) {
      return Response.json({ error: 'Data staff tidak valid', details: parsed.error.flatten() }, { status: 400 });
    }
    ...
    const staff = await transaction(async (client) => {
      ...
      if (!resort.rows[0]) throw new ApiError(400, 'Resort tidak ditemukan');
    ...
    if (error.code === '23505') {
      return Response.json({ error: 'Email sudah digunakan oleh akun lain.' }, { status: 409 });
    }
    return jsonError(error);
  }
}
