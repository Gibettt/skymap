...
export async function PATCH(request, { params }) {
  try {
    ...
    if (!parsed.success) {
      return Response.json({ error: 'Data pengguna tidak valid', details: parsed.error.flatten() }, { status: 400 });
    }
    ...
    const updated = await transaction(async (client) => {
      ...
      if (resortIds.length) {
        const resorts = await client.query(
          'SELECT id FROM resorts WHERE id = ANY($1::uuid[]) ORDER BY id FOR UPDATE',
          [resortIds]
        );
        if (data.resortId && !resorts.rows.some((resort) => resort.id === data.resortId)) {
          throw new ApiError(400, 'Resort tidak ditemukan');
        }
      }
      ...
      if (removedRole) {
        ...
        if (coverage.replacement_count === 0 && coverage.open_bookings_count > 0) {
          throw new ApiError(409, 'Staff terakhir tidak dapat dipindahkan atau dinonaktifkan selama resort masih memiliki booking terbuka.');
        }
      }

      const { rows } = await client.query(
        `UPDATE users SET name = $2, email = $3, phone = $4, role = $5,
          status = $6, resort_id = $7 WHERE id = $1 RETURNING *`,
        [id, data.name, data.email, data.phone || null, data.role, data.status,
    ...
    if (error.code === '23514') {
      return Response.json({ error: 'Perubahan ditolak karena staff ini masih dibutuhkan untuk booking resort yang terbuka.' }, { status: 409 });
    }
