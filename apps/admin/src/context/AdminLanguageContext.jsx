...
const literalIdToEn = {
  ...
  'Presence Staff per Resort': 'Staff Presence by Resort',
  'Pilih resort untuk memfilter staff internal dan external.': 'Select a resort to filter internal and external staff.',
  'Semua Resort': 'All Resorts',
  'Realtime tersambung': 'Realtime connected',
  'Mode polling': 'Polling mode',
  'Nama & Email': 'Name & Email',
  Akun: 'Account',
  'Terakhir Aktif': 'Last Seen',
  'Tidak dipantau': 'Not monitored',
  'Belum terdeteksi': 'Not detected yet',
  'Baru saja': 'Just now',
  ...
  'Jadwal saat ini': 'Current schedule',
  'Tanggal baru': 'New date',
  ...
  'Alasan akan tersimpan di riwayat perubahan booking.': 'The reason will be saved in the booking change history.',
  'Waktu selesai harus setelah waktu mulai.': 'End time must be after start time.',
  'Simpan jadwal baru': 'Save new schedule',
  ...
  'Kelola akun, peran, dan hak akses seluruh pengguna sistem SpaceCat ASTROTOURISM.': 'Manage accounts, roles, and access rights for all SpaceCat ASTROTOURISM users.',
  ...
  'Update Data SpaceCat ASTROTOURISM': 'SpaceCat ASTROTOURISM Data Update',
...
function translateLiteral(value, language) {
  ...
  if (!translated && language === 'en') {
    translated = trimmed
      ...
      .replace(/^(\d+) staff terdaftar$/, '$1 registered staff')
      .replace(/^(\d+) menit lalu$/, '$1 minutes ago')
      .replace(/^(\d+) jam lalu$/, '$1 hours ago')
    ...
    translated = trimmed
      ...
      .replace(/^(\d+) registered staff$/, '$1 staff terdaftar')
      .replace(/^(\d+) minutes ago$/, '$1 menit lalu')
      .replace(/^(\d+) hours ago$/, '$1 jam lalu')
