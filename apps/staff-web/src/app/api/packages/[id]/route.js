...
const PACKAGE_COLUMNS = `
  ...
  CASE WHEN image_data IS NULL THEN NULL ELSE CONCAT('/api/packages/', id, '/image') END AS image_url,
  ...
  try {
    ...
    const updated = await transaction(async (client) => {
      ...
      if (body.childPriceUsd !== undefined) {
        childPriceUsd = body.childPriceUsd === null || body.childPriceUsd === '' ? null : Number(body.childPriceUsd);
      }
  ...
  try {
    ...
    const deleted = await transaction(async (client) => {
      ...
      if (bookingCount.rows[0].count > 0) {
        throw new ApiError(
          409,
          `This package is used by ${bookingCount.rows[0].count} bookings and cannot be deleted. Deactivate it instead.`,
        );
      }
    ...
    return Response.json({ success: true, package: deleted });
  } catch (error) {
    return jsonError(error);
  }
}
