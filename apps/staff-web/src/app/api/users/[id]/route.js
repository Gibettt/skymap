...
export async function PATCH(request, { params }) {
  try {
    ...
    const updated = await transaction(async (client) => {
      ...
      if (removedRole) {
        ...
        if (coverage.replacement_count === 0 && coverage.open_bookings_count > 0) {
          throw new ApiError(409, 'The last covered staff member cannot be moved or deactivated while the resort has open bookings.');
        }
      }
      ...
      const { rows } = await client.query(
        ...
        [id, data.name, data.email.toLowerCase(), data.phone || null, data.role, data.status,
    ...
    if (error.code === '23514') {
      return Response.json({ error: 'This change was rejected because the staff member is still required for open resort bookings.' }, { status: 409 });
    }
  ...
  try {
    ...
    if (error.code === '23503') {
      return Response.json(
        { error: 'This user is linked to operational history and cannot be deleted. Set the account to inactive instead.' },
        { status: 409 },
      );
    }
    if (error.code === '23514') {
