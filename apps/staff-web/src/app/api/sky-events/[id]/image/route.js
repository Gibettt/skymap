  ...
  try {
    ...
    const { rows } = await query(
      `SELECT image_data, image_mime_type FROM sky_events
       WHERE id = $1 AND resort_id = $2 AND image_data IS NOT NULL`,
      [id, user.resort_id],
    );
  ...
  try {
    ...
    const updated = await transaction(async (client) => {
      const before = await client.query(
        'SELECT id, image_file_name FROM sky_events WHERE id = $1 AND resort_id = $2 FOR UPDATE',
        [id, user.resort_id],
      );
      if (!before.rows[0]) return null;
      await client.query(
        `UPDATE sky_events SET image_data = $3, image_mime_type = $4, image_file_name = $5,
          image_url = NULL, updated_by = $6 WHERE id = $1 AND resort_id = $2`,
        [id, user.resort_id, imageData, file.type, fileName, user.id],
      );
    ...
    const updated = await transaction(async (client) => {
      ...
      await client.query(
        `UPDATE sky_events SET image_data = NULL, image_mime_type = NULL, image_file_name = NULL,
          image_url = NULL, updated_by = $3 WHERE id = $1 AND resort_id = $2`,
        [id, user.resort_id, user.id],
      );
      await writeAudit(client, {
        actorId: user.id,
