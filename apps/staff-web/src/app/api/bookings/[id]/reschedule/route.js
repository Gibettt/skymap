import { assertSameOrigin, ApiError, jsonError, parseJsonBody, requirePermission, writeAudit } from '@ephemeris/auth';
import { transaction, refreshAfterBookingChange } from '@ephemeris/db';
import { canRescheduleBookingStatus, rescheduleBooking } from '@ephemeris/db/bookings';
import { canRescheduleBooking } from '@ephemeris/db/scopes';
...
export async function POST(request, { params }) {
  try {
    ...
    const booking = await transaction(async (client) => {
      ...
      if (!canRescheduleBooking(user, before)) throw new ApiError(403, 'Forbidden');
      if (!canRescheduleBookingStatus(before.status)) {
        throw new ApiError(409, `Booking with status ${before.status} cannot be rescheduled`);
      }
