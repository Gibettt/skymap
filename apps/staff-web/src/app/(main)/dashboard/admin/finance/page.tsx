...
import { getFinance } from "../_lib/admin-data";
...
export default async function FinancePage() {
  ...
  const payoutWallet = finance.payouts.slice(0, 2).map((payout) => ({
    id: payout.id,
    label: `${payout.requester_name} · ${statusLabel(payout.status)}`,
    description: `${payout.bank_name} · ${currency(payout.amount_usd)}`,
  }));
  const payoutTransactions = finance.payouts.slice(0, 3).map((payout) => ({
    id: payout.id,
    title: `${payout.requester_name} · ${statusLabel(payout.status)}`,
    date: `${payout.resort_name ?? "Internal"} · ${format(new Date(payout.created_at), "MMMM dd, yyyy")}`,
  }));
  const totalPayoutAmount = finance.payouts.reduce((total, payout) => total + numberValue(payout.amount_usd), 0);
  const contacts = finance.payouts.slice(0, 4).map((payout) => ({
    id: payout.id,
      ...
      <Tabs defaultValue="30-days" className="flex flex-col gap-4">
        ...
        <TabsContent value="30-days" className="flex flex-col gap-4">
          ...
          <div className="grid grid-cols-1 gap-4 xl:grid-cols-12">
            ...
            <div className="xl:col-span-4" id="payouts">
              <UpcomingTransactions
                transactions={payoutTransactions}
                totalAmount={totalPayoutAmount}
                highlightAmount={numberValue(summary.open_payout_total)}
                count={finance.payouts.length}
                summaryLabel="payout requests recorded"
