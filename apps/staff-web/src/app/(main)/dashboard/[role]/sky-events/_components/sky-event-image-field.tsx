...
export type SkyEventImageValue = File | null | undefined;
...
export function SkyEventImageField({
  currentImageUrl,
  value,
  onChange,
}: {
  currentImageUrl?: string | null;
  value: SkyEventImageValue;
  onChange: (value: SkyEventImageValue) => void;
}) {
  const inputId = useId();
  ...
  return (
    <Field data-invalid={Boolean(error)}>
      ...
      {visibleUrl ? (
        <Attachment className="w-fit">
          <AttachmentMedia variant="image">
            ...
            <img src={visibleUrl} alt="Event preview" />
          </AttachmentMedia>
          <AttachmentContent>
            <AttachmentTitle>{value instanceof File ? value.name : "Current image"}</AttachmentTitle>
            <AttachmentDescription>
              {value instanceof File ? formatFileSize(value.size) : "Displayed on the public landing page"}
            </AttachmentDescription>
          </AttachmentContent>
          <AttachmentActions>
            <AttachmentAction type="button" aria-label="Change image" onClick={() => inputRef.current?.click()}>
              <RefreshCw />
            </AttachmentAction>
      ...
      <FieldDescription>JPG, PNG, or WEBP up to 2MB. Displayed on the public landing page.</FieldDescription>
