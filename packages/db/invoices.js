...
const CUSTOMER_ELIGIBLE_STATUSES = new Set([
	"active",
	"rescheduled",
	"completed",
]);
...
export function invoiceNumberFor(type, id, issuedAt = new Date()) {
	...
	if (Number.isNaN(date.getTime()))
		throw new TypeError("Invalid invoice issue date");
...
export function buildCustomerLineItems(booking, experiences = []) {
	...
	return sourceItems.map((experience, index) => ({
		...
		description: String(
			experience.package_name ?? booking.package_name ?? "Ephemeris experience",
		),
		detail: [
...
export async function selectInvoiceById(client, id) {
	const { rows } = await client.query(
		`${invoiceSelect} WHERE i.id = $1 LIMIT 1`,
		[id],
	);
...
async function selectInvoiceBySource(client, type, sourceId) {
	const sourceColumn =
		type === "staff_payout" ? "payout_request_id" : "booking_id";
	const { rows } = await client.query(
		`${invoiceSelect} WHERE i.${sourceColumn} = $1 LIMIT 1`,
		[sourceId],
	);
	return normalizeInvoice(rows[0]);
}
...
export async function issueCustomerInvoice(
	client,
	{ bookingId, issuedById, resortId = null },
) {
	...
	await client.query(
      ...
      service_charge_usd, tax_usd, total_usd, line_items, source_snapshot,
      notes, issued_by
    ) VALUES (
	...
	await client.query("UPDATE bookings SET invoice_number = $2 WHERE id = $1", [
		booking.id,
		invoiceNumber,
	]);
...
export async function issuePayoutInvoice(
	client,
	{ payoutRequestId, issuedById },
) {
	const { rows } = await client.query(
	...
	const existing = await selectInvoiceBySource(
		client,
		"staff_payout",
		payoutRequestId,
	);
	...
	await client.query(
      ...
      0, 0, $11, $12::jsonb, $13::jsonb, $14, $15
    )`,
		[
			id,
			invoiceNumber,
...
export async function listInvoices(client) {
	const { rows } = await client.query(
		`${invoiceSelect} ORDER BY i.issued_at DESC, i.created_at DESC LIMIT 500`,
	);
 ...
 * @param {{ resortId?: string | null }} [options]
 */
export async function listCustomerPaymentWorkflows(
	client,
	{ resortId = null } = {},
) {
	const resortScope = resortId ? " AND b.resort_id = $1" : "";
