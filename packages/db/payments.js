const CUSTOMER_PAYMENT_ELIGIBLE_STATUSES = new Set([
	"active",
	"rescheduled",
	"completed",
]);
...
export function validateCustomerPaymentConfirmation(
	...
	if (!CUSTOMER_PAYMENT_ELIGIBLE_STATUSES.has(booking.status)) {
		return {
			error:
				"Only active, rescheduled, or completed bookings can receive payment.",
			status: 409,
		};
	}
	...
	const paymentMethod = normalizeCustomerPaymentMethod(
		requestedPaymentMethod ?? booking.payment_method,
	);
	if (!paymentMethod) {
		return {
			error:
				"Set the booking payment method to Bank transfer or Cash before confirming payment.",
			status: 409,
		};
	}
	if (
		paymentMethod === "Bank transfer" &&
		!String(reference ?? booking.payment_reference ?? "").trim()
	) {
...
export async function confirmCustomerPayment(
	...
	const { rows } = await client.query(
		`SELECT * FROM bookings WHERE id = $1${bookingScope} FOR UPDATE`,
		bookingParams,
	);
	...
	const updateScope = resortId ? " AND resort_id = $6" : "";
	const updateParams = [
		bookingId,
	...
	if (!updated.rows[0]) {
		return {
			error:
				"Payment status changed while it was being confirmed. Refresh and try again.",
			status: 409,
		};
	}
