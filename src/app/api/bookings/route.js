...
export async function GET() {
  try {
    ...
    const where = user.role === 'external' ? 'WHERE b.staff_id = $1' : '';
    const params = user.role === 'external' ? [user.id] : [];
    const { rows } = await query(`${bookingSelect} ${where} ORDER BY b.event_date DESC, b.created_at DESC`, params);
...
export async function POST(request) {
  try {
    ...
    const created = await transaction(async (client) => {
      ...
      const staff = await client.query('SELECT id, role FROM users WHERE id = $1 AND status = $2', [staffId, 'active']);
      ...
      const status = user.role === 'external' ? 'pending_review' : 'booked';
      const { rows } = await client.query(
        `INSERT INTO bookings (
          ...
          special_occasion, booking_source, package_id, add_ons, package_notes,
          staff_id, status, signed_by_guest, notes,
