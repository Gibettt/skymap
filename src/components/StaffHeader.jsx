...
export default function StaffHeader({ role = 'Internal', onNewBooking }) {
  ...
  const getPageInfo = () => {
    if (pathname.includes('/observations/')) {
      ...
      return { title: exp?.title ?? 'Observations', crumb: 'Observations' };
    }
    if (pathname.endsWith('/bookings')) return { title: role === 'External' ? 'My Bookings' : 'Booking Saya', crumb: 'Reservasi' };
    if (pathname.endsWith('/jadwal')) return { title: role === 'External' ? 'Calendar' : 'Jadwal Observasi', crumb: 'Penjadwalan' };
